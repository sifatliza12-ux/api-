...
// Same helper workflowController.parameterize uses to mirror a freshly
// recorded workflow into the My APIs store — reused here (not
// reimplemented) so an AI-generated workflow shows up in My APIs, and is
// runnable/publishable through the existing endpoints, exactly like a
// manually recorded one. Without this, generateWorkflow would only ever
// create a `workflows` row, which the My APIs / Run / Publish UI never
// reads from directly (it reads `my_apis` rows for endpoint/method/price).
const { buildMyApiRecord } = require('./myApisController');
// The SAME parameterization function workflowController.parameterize uses
// for a manually recorded workflow — see aiBrowserAgent.js's own doc
// comment: its `steps` output is raw recorded events (not already
// placeholder-substituted), specifically so this single, existing function
// remains the one place that decides parameter names/placeholders, for
// both recording paths. Not reimplemented, not duplicated.
...
const generateWorkflow = async (req, res) => {
  try {
    ...
    // result.steps are RAW RECORDED EVENTS (see aiBrowserAgent.js's own doc
    // comment) — running them through the SAME parameterizer a manual
    // recording goes through (workflowController.parameterize) is what
    // turns them into real {{placeholder}} steps plus a matching
    // parameters array, in lockstep, exactly like a human recording. This
    // deliberately replaces intent.parameters (the user's stated request)
    // as the source of truth for workflow.parameters: intent.parameters
    // describes what was ASKED for, not necessarily what ended up as an
    // actual, substitutable value in the recorded steps — using the
    // parameterizer's own result is what keeps "what the UI lets you edit"
    // truthfully wired to "what replay will actually use".
    const { parameters, steps } = result.steps.length
      ? parameterizeWorkflowRuleBased(result.steps)
      : { parameters: [], steps: [] };
    ...
    // Mirror into the My APIs store — same call, same shape
    // workflowController.parameterize already uses for a manually recorded
    // workflow — so this one is immediately visible/runnable/publishable
    // through the existing My APIs UI with no separate code path.
