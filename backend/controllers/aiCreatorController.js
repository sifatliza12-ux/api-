...
const generateWorkflow = async (req, res) => {
  try {
    ...
    // intent.parameters carries the confirmed name/type/value/label/
    // description the user actually saw on the confirmation screen — real
    // backend data, not a frontend guess. Recorded as this workflow's
    // parameters purely for display/editing (see aiBrowserAgent.js's own
    // doc comment: recorded steps currently carry concrete, already-typed
    // values rather than {{placeholder}} tokens, so re-editing a value here
    // and re-running does not yet change the live site interaction — full
    // parameterization of agent-driven runs is a later phase, same
    // limitation this endpoint already has for its own step output).
